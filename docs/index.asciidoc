...
Generally speaking, Java stack trace elements with no source information are dynamically generated classes.

So in most cases it's a wise choice to exclude them from the digest computation (and by the way much faster than evaluating
...
For more information about the https://github.com/pismy/logstash-filter-java_stack_digest/blob/master/stack-digest.md[stack digest stability challenge].
